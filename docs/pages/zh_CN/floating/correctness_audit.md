...
官方语料是外部固定输入，不进入版本库。unsupported 与 diagnostic 会保留在汇总中；需要把它们作为失败时启用 strict mode。详见[一致性测试流程](https://github.com/Luna-Flow/floating/blob/main/testdata/decimal/README.md)。
