...
- `@immut.Vector` 一致采用值语义：`set`、`map`、`left_scale`、`right_scale` 等操作都会返回新值。
- 这个包会尽量和 `@mutable.Vector` 保持共享代数子集的一致性，但不会提供原地修改 API。
- 优化后的 core 不可变向量实现已经成为整个库存储策略的一部分，而不只是内部细节，因为它支撑了更统一的扁平化数据路径。
