...
`algebra` は実験的機能です。バックエンド接続の試行とフィードバック収集には利用できますが、
trait 階層、親 trait の要件、演算子の契約、関数シグネチャは、安定化までに互換性なく
変更される可能性があります。下流ライブラリは必要最小限の能力だけに依存し、現時点では
このパッケージを互換性の安定した公開境界として再公開しないでください。
...
外部型の作者は、最小の trait レベルと演算子の約束を選ぶため、まず
[エコシステム接続ガイド](/ja_JP/linear-algebra/algebra/integration) を参照してください。
